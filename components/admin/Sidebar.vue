<template>
  <div class="relative  top-0 left-0 w-full min-h-screen flex flex-col">
    <!-- App name with logo -->
    <div
      class="relative h-24 font-mono font-bold text-2xl flex justify-center items-center gap-6"
    >
      <img :src="logoHeader.imgUrl" alt="" class="w-8 h-8" />
      <h2>{{ appName }}</h2>
    </div>
    ...
    <div class="relative top-12 flex flex-col justify-center items-center">
      <ul class="relative flex flex-col gap-5">
        ...
        >
          <a
            class="flex justify-start items-center gap-6 p-3 px-6 rounded-md transition ease-in-out"
          >
            <img :src="elem.imgUrl" alt="" class="w-4 h-4" />
            <h3 class="font-mono">{{ elem.textContent }}</h3>
          </a>
        </li>
        <div class="indicator_box transition duration-300 z-10 rounded-lg absolute first-line:absolute top-0 left-0 w-full h-12 bg-black"></div>
      </ul>
    </div>
...
<script setup>
import { sidebar, logoHeader, appName } from "../../constants/index";
