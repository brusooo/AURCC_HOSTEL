                   ...
                   <div class="flex flex-row gap-3 lg:gap-8 ">
                    ...
                    <div class="flex flex-col lg:flex-row lg:gap-3" >from:
                      ...
                      <div class="flex flex-col lg:flex-row lg:gap-3">to:
                        ...
                        <li class="text-slate-700 text-sm pt-0.5 w-16"> {{ item.reason_ }}</li>
                   </div>
            
          
                  <div class="flex gap-4 lg:pt-4">
                                                        <button @click="updatePost(item.id, { id: item.id,stuid_:item.stuid_, from_: item.from_, to_: item.to_, accepted_by: 'hodaccepted', reason_: item.reason_, informed_parent_on: item.informed_parent_on })"
                                                            class=" bg-indigo-500 hover:bg-indigo-600 text-white font-bold  py-1 px-6 rounded">accept</button>
                                                            <button @click="updatePost(item.id, { id: item.id, stuid_: item.stuid_, from_: item.from_, to_: item.to_, accepted_by: 'hoddenied', reason_: item.reason_, informed_parent_on: item.informed_parent_on })"
        ...
        <div v-if="accepted == 'accepted'" class="">
            ...
            <ul v-for="item in data1" :key="item.id" class="flex flex-col lg:flex-row lg:gap-5 gap-3 p-4 bg-white shadow-lg shadow-slate-400">
                       <div class="flex flex-row gap-3 lg:gap-8 ">
                        ...
                        <li class="text-slate-700 text-sm pt-0.5 w-16"> {{ item.reason_ }}</li></div>
                            <div v-if="item.accepted_by == 'hodaccepted'" class=" text-sm w-20">accepted by HOD</div>
                            <div v-if="item.accepted_by == 'wadernaccepted'" class="text-sm w-20">accepted by Wadern</div>
                            <div v-if="item.accepted_by == 'waderndenied'" class="text-sm w-20">denied by wadern</div>
                    ...
                    <div v-if="!accepted">no items....</div>
           </div>
...
// reactive state
const { data } = await useFetch('http://127.0.0.1:8000/users/accepted')

const { data : data1 } = await useFetch('http://127.0.0.1:8000/users/acceptedhod')

const { data : data2 } = await useFetch('http://127.0.0.1:8000/users/deniedhod')
console.log(data)
console.log(data1)
console.log(data2)
// upadte values
