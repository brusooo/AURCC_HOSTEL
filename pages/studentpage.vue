<template>
    ...
    <div v-else class="relative w-full xl:h-screen flex flex-col overflow-y-scroll">

        <!-- navbar -->
        <nav class="relative w-full h-[10%] bg-crimson flex justify-center items-center">
            ...
            <div class="relative flex translate-x-7 max-md:hidden">
                <h1 class="sm:text-3xl text-xl font-mono font-bold">Students Profile</h1>
            </div>
            <!-- user image -->
            <div class="relative ml-auto -left-9 flex justify-center items-center gap-6">
                <h1 class="sm:text-base text-xs font-mono max-md:hidden">Welcome, Bursooo</h1>
                <div class="relative w-10 h-10  flex justify-center items-center rounded-full overflow-hidden">
                    <img src="/images/sidebar/user.jpg" alt="" />
                </div>
            </div>
        </nav>

        <div class="relative w-full  h-[90%] xl:my-0 my-5 flex justify-center items-center">
            <StudentsProfile :generalDetails="generalDetails" :otherDetails="otherDetails"
...
const getStudentDetails = async (active_reg_no) => await useFetch(
